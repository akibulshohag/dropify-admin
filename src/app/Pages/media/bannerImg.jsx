...
const SliderImages = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        let res = await axios.get("/content/banner/list");
        if (res?.data?.success) {
  ...
  const formSubmitHandler = async (event) => {
    ...
    try {
      ...
      setIsLoading(true);
      const res = await axios.post(`/content/banner/add`, obj);
      if (res?.data?.success) {
        setFileList([]);
  ...
  const deleteHandler = async () => {
    try {
      let res = await axios.post(`/content/banner/delete`, {bannerId:deleteData?.bannerId});
      setDeleteData(null);
      setisRender(!isRender)
      openNotificationWithIcon(res?.data?.message, "success");
    } catch (error) {
  ...
  const featuredUpdateHandler = async (data) => {
    try {
      const createRes = await axios.post(`/content/banner/update`, {
        bannerId: data?.bannerId,
        isActive: !data?.isActive,
      });
      let updatedData = dataList.map((list) => {
        if (list.bannerId === data.bannerId) {
  ...
  return (
    <div className="m-sm-30">
      <div className="mb-sm-30">
        <Breadcrumb routeSegments={[{ name: "Banner Image" }]} />
      </div>

      <Grid container>
        <Grid item md={6} xs={12} className="mb-4">
          <Card elevation={3}>
            <CardHeader title="Add New Banner Image" />
        ...
        <Grid item md={10} xs={12}>
          <Card elevation={3}>
            <CardHeader title="All Banner Images" />
            {!isPageLoading ? (
