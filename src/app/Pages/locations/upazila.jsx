...
const UpazilaList = () => {
  ...
  useEffect(() => {
    if (districtId) {
      setUpazilaList(
        dataList?.upazilas.filter((i) => i.districtId === districtId)
      );
    }
  }, [districtId, dataList]);
  ...
  const deleteHandler = async () => {
    try {
      let res = await axios.delete(`/location/remove-upazila/${deleteId}`);
  ...
  const addUpazilaHandler = async () => {
    try {
      ...
      let res = await axios.post(`/location/add-upazila`, obj);
      setUpazilaName("");
  ...
  return (
    <div className="m-sm-30">
      <div className="mb-sm-30">
        <Breadcrumb routeSegments={[{ name: "upazila" }]} />
      </div>{" "}
      <Grid container>
        <Grid item md={6} sm={8} xs={12}>
          <Card elevation={3}>
            <CardHeader title="Upazila List" />
