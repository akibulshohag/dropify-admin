...
const CreateProduct = () => {
  ...
  useEffect(() => {
    let fetchData = async () => {
      try {
        const [categoryData, brandData, attributeData, stickerData] = await Promise.all([
  ...
  const formSubmitHandler = async (data) => {
    try {
      ...
      let res = await axios.post("/product/create", obj);
      /*
      reset();
      setDescription("");
      setCategoryIds([]);
      ...
      <Grid container className="mb-6">
        ...
        <Grid item md={10} xs={12}>
          <form onSubmit={handleSubmit(formSubmitHandler)}>
                ...
                <Grid container spacing={1} alignItems="center" className="mb-2">
                  ...
                  <Grid item sm={8} xs={12}>
                    <Autocomplete
                      ...
                      onChange={(e, val) => setCategoryIds(val.map((v) => v._id))}
                      size="medium"
                      name="category"
            ...
            >
              {isLoading ? <CircularProgress size={24} color="inherit" /> : "Submit Product"}
            </Button>
          </form>
        </Grid>
      </Grid>
