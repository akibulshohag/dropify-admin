...
const UpdateProduct = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setIsLoading(true);
        const [categoryData, brandData, attributeData, stickerData, res] = await Promise.all([
          axios.get("/category/fetch-all"),
          axios.get("/brand/fetch-all"),
          axios.get("/attribute/fetch-all"),
          axios.get("/sticker/fetch-all"),
          axios.get(`/product/single-product/${productSlug}`),
        ]);
        ...
        setValue(
          "unitPrice",
          Number(
            data?.nonVariationPurchase?.totalPrice / data?.nonVariationPurchase?.totalQuantity
          ) || 0
        );
  ...
  const formSubmitHandler = async (data) => {
    try {
      ...
      let obj = {
        ...
        nonVariation: {
          stock: isVariant ? 0 : stock || 0,
          unitPrice: isVariant ? 0 : data?.unitPrice || 0,
      ...
      setBtnLoading(true);
      let res = await axios.patch(`/product/update/${productData?._id}`, obj);
      if (res?.data?.success) {
  ...
  const attributeSelectHandler = (event, value) => {
    ...
    // console.log("attributeSetArray: ", attributeSetArray);
  ...
  const updateNestedAttributeSelete = (event, value, attrId) => {
    ...
    let filterAttributeList = updateAttributeOptions.filter((f) => f?.options.length !== 0);

    let makeVarations = [];

    if (filterAttributeList.length > 0) {
      let combinationResult = getCombinations(filterAttributeList.map((u) => u.options));
            ...
            <form onSubmit={handleSubmit(formSubmitHandler)}>
              ...
              <Card className="elevation-z4 mb-6">
                ...
                <CardContent>
                  ...
                  {isVariant && oldVariationList.length > 0 && (
                    <div className="mb-6">
                      <Typography variant="h6" className="text-center text-green mt-8 mb-6">
                      ...
                      <Table className="whitespace-pre">
                        <TableHead>
                          <TableRow>
                            ...
                            <TableCell className="min-w-100">Regular P.</TableCell>
                            <TableCell className="min-w-100">Discount T.</TableCell>
                            <TableCell className="min-w-100">Discount A.</TableCell>
                            <TableCell className="min-w-100">Sale P.</TableCell>
                  ...
                  {isVariant ? (
                    <>
                      {/* {productData?.isUsed ? ( */}
                      <Typography variant="h6" className="text-center text-secondary mt-8 mb-6">
                        Add More Variations
                      </Typography>
                      {/* ) : (
                        <Typography variant="h6" className="text-center text-green mt-8 mb-6">
                      ...
                      <Grid container spacing={1} alignItems="center" className="mb-8">
                        <Grid item sm={3} xs={12}>
                      ...
                      {selectedAttributeSet.map((attr, idx) => (
                        <Grid key={idx} container spacing={1} alignItems="center" className="mb-2">
                          <Grid item sm={3} xs={12}>
                          ...
                          <Grid item sm={8} xs={12}>
                            <Autocomplete
                              ...
                              isOptionEqualToValue={(option, value) => option._id === value._id}
                              onChange={(e, val) =>
                                updateNestedAttributeSelete(e, val, attr[0].attributeId)
                              }
                              value={
                        ...
                        <Grid item sm={2} xs={12}>
                          <TextField
                            ...
                            onChange={(e) => {
                              ...
                              if (e.target.value === "FLAT" && regularPrice < discountAmount) {
                                setDiscountAmount(0);
                              }
                            }}
                          >
              ...
              <Card className="elevation-z4 mb-6">
                ...
                <CardContent>
                  ...
                  <ChartPage chartList={chartList} setChartList={setChartList} />
                </CardContent>
              </Card>
                    ...
                    <Grid item sm={6}>
                      <Box>
                        <FormControlLabel
                          ...
                          label={`Free Delivery (${isFreeDelivery ? "Enable" : "Disable"})`}
                        />
                      </Box>
                      <Box>
                        <FormControlLabel
                          ...
                          label={`Feature Product (${isFeatured ? "Enable" : "Disable"})`}
                        />
                      </Box>
                      <Box>
                        <FormControlLabel
                          control={
                            <IOSSwitch
                              ...
                              onChange={(event) => setIsOwnDisabled(!event.target.checked)}
                            />
                          }
                          label={`Product Status (${!isOwnDisabled ? "Enable" : "Disable"})`}
                        />
                      </Box>
                      <Box>
                        <FormControlLabel
                          control={
                            <IOSSwitch
                              ...
                              onChange={(event) => setIsPosSuggest(event.target.checked)}
                              value={isPosSuggest}
                            />
                          }
                          label={`Pos Suggestion (${isPosSuggest ? "Enable" : "Disable"})`}
                        />
                      </Box>
                    </Grid>
                  </Grid>
              ...
              >
                {btnLoading ? <CircularProgress size={24} color="inherit" /> : "Update Product"}
              </Button>
            </form>
          </Grid>
        </Grid>
